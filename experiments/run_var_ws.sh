...
exper_data_dir="results/exper_var_ws/fixed_ws=${fixed_ws}m"
rm -rf "${exper_data_dir}"
fixed_ckpt_path="ckpt/n=${fixed_ws}m,k=16,v=500,d=zipf:0.99"

for var_ws in 1 2 3 4 5 6 7 8 10 12 14 16; do
  data_dir="${exper_data_dir}/var_ws=${var_ws}m"
  var_ckpt_path="ckpt/n=${var_ws}m,k=16,v=500,d=zipf:0.99"

  uv run -m driver.run_mp pipeline \
    n=${var_ws}m,k=16,v=500,w=0.05,d=unif \
    n=${fixed_ws}m,k=16,v=500,w=0.05,d=unif \
    --tables "${tables[@]}" \
    --num_clients 16 \
    --async_queue_depth 2 \
    --mrc_salt 1% \
  ...
  uv run -m scripts.plot_var "${exper_data_dir}" \
    --xlabel "\$T_1\$ working set (M keys)" --xscale M --xticks 1 4 8 12 16
done
